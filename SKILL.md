---
name: stellar-webmcp-trust-router
description: Safely discover, fingerprint, classify, validate, confirm, execute, and receipt WebMCP tools exposed by visible browser pages. Use when an agent interacts with document.modelContext, WebMCP-enabled websites, browser-side getTools()/executeTool() bridges, risky web actions such as payments or identity changes, schema drift detection, or optional Stellar on-chain receipt anchoring.
---
...
| Create an audit receipt | Hash inputs and outputs, record risk and confirmation status | `references/stellar-receipts.md` and `assets/receipt-schema.json` |
| Anchor a receipt on Stellar / prove this action on-chain | Optional Foundry `cast send` anchoring after Stellar write pre-checks | `references/stellar-receipts.md#optional-stellar-anchoring` |
...
12. Optionally anchor the receipt hash on Stellar only after the user approves the gas-spending write and all Stellar pre-checks pass.
...
Before optional Stellar anchoring:
...
5. Confirm `RPC` points to the intended Stellar network.
...
- `.env.example`: copy to `.env` and add a funded Stellar `PRIVATE_KEY`; `.env` is intentionally gitignored.
- `assets/stellar-anchor-receipt.sh`: Foundry script that loads `.env` and sends a real Stellar Testnet zero-value memo transaction with the receipt hash as calldata.
...
- Execute this approved WebMCP action, create an audit receipt, and anchor the receipt hash on Stellar.

For Stellar anchoring, run `assets/stellar-anchor-receipt.sh` with a funded `PRIVATE_KEY` after the user approves the on-chain write.
