...
RECEIPT_HASH="${1:-}"
RPC="${RPC:-https://stellar-testnet.example.com}"
EXPECTED_CHAIN_ID="${EXPECTED_CHAIN_ID:-stellar-testnet}"

if [[ ! "$RECEIPT_HASH" =~ ^0x[0-9a-fA-F]{64}$ ]]; then
  echo "Usage: ./assets/stellar-anchor-receipt.sh 0x<64-hex-receipt-hash>" >&2
  exit 2
fi
