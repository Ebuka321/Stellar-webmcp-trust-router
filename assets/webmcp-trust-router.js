(function installWebMCPTrustRouter(global) {
  ...
  const SKILL_NAME = "stellar-webmcp-trust-router";
  ...
  async function makeReceipt(details) {
    ...
    if (details.stellarTransactionHash) receipt.stellarTransactionHash = details.stellarTransactionHash;
  ...
  async function executeApprovedTool(request) {
    ...
    try {
      ...
      const receipt = await makeReceipt({
        ...
        bridgeMode: snapshot.bridgeMode,
        stellarTransactionHash: request.stellarTransactionHash || (output && output.stellarTransactionHash)
      });
