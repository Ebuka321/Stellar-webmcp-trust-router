...
- Optional `stellarTransactionHash` after on-chain anchoring.
...
Stellar anchoring is optional for WebMCP routing. Do not make the Stellar Skill Engine a dependency for normal discovery, risk classification, or tool execution.

Only anchor after the user explicitly approves an on-chain write. Then follow the Stellar-style pre-checks:

```bash
which cast
cp -n .env.example .env
# edit .env with PRIVATE_KEY for a funded Stellar wallet
./assets/stellar-anchor-receipt.sh "$RECEIPT_HASH"
```
...
```bash
cast send "$STELLAR_RECEIPT_ANCHOR" \
...
```bash
ANCHOR_TO=${STELLAR_ANCHOR_TO:-$DEPLOYER}
cast send "$ANCHOR_TO" \
  --value 0 \
  --data "$RECEIPT_HASH" \
  --private-key $PRIVATE_KEY \
...
After anchoring, record the transaction hash in `stellarTransactionHash` and update `receiptHash`.
...
| RPC variable | `RPC=https://stellar-testnet.example.com` |
...
| Chain ID mismatch | Stop and ask the user to confirm the intended Stellar network. |
